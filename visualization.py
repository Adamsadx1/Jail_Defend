...
def compare_entropy_kde(df1, df2, class_1_label='normal', class_2_label='malicous'):
    # 提取两个类别的 entropy 列
    normal = df1['entropy']
    malicious = df2['entropy']
    normal = normalize_entropy(normal)
    malicious = normalize_entropy(malicious)

    plt.figure(figsize=(8, 6))
    sns.kdeplot(normal, color='blue', fill=True, label=class_1_label, alpha=0.5)
    sns.kdeplot(malicious, color='red', fill=True, label=class_2_label, alpha=0.5)

    #plt.yscale('log')
    plt.title('Entropy Distribution (KDE)')
    plt.xlabel('Entropy')
    plt.ylabel('Density')
    plt.legend()
    plt.tight_layout()
